...
import numpy as np
import functools
from .base_blocks import Conv1x1, Basic2d, BasicBlock, weights_init, inplace_relu, GenKernel
from .refine_modules import PMP
...
class Net(nn.Module):
    ...
    def __init__(self, block=BasicBlock, bc=16, img_layers=[2, 2, 2, 2, 2, 2],
        ...
        self.pred5 = PMP(level=5, in_ch=bc*16, out_ch=bc*16, drop_path=drop_path, up=False, pre_dilation=1)
        self.pred4 = PMP(level=4, in_ch=bc*16, out_ch=bc*16, drop_path=drop_path, pre_dilation=1)
        self.pred3 = PMP(level=3, in_ch=bc*16, out_ch=bc*16, drop_path=drop_path, pre_dilation=1)
        self.pred2 = PMP(level=2, in_ch=bc*16, out_ch=bc*8,  drop_path=drop_path, pre_dilation=1) 
        self.pred1 = PMP(level=1, in_ch=bc*8,  out_ch=bc*4,  drop_path=drop_path, pre_dilation=2)
        self.pred0 = PMP(level=0, in_ch=bc*4,  out_ch=bc*2,  drop_path=drop_path, pool=False, pre_dilation=2) 
